...
planRouter.post("/generate", async(req: Request, res:Response)=>{
    try {
        ...
        if (!profile){
            return res
            .status(400)
            .json({ error: "Пользователь не найден. Для начала пройдите регистрацию."})
        }

        const latestPlan = await prisma.trainig_plans.findFirst({
            where:{user_id:userId},
        ...
        const newPlan = await prisma.trainig_plans.create({
            data:{
                user_id: userId,
...
planRouter.get("/current", async(req: Request, res:Response)=>{
    try {
        ...
        const plan = await prisma.trainig_plans.findFirst({
            where:{ user_id: userId },
            orderBy: { created_at: "desc" },
        });
