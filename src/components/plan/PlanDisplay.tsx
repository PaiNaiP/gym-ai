...
function ExersuceRow({exercise, index,}:{
    exercise:Exercise;
    index:number;
}){
    return (
        <tr className="border-b border-[var(--color-bordered)] last:border-0">
            ...
            <td className="py-3 px-4 text-center">
                <span className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-sm font-medium ${exercise.rpe >=8 ?
        ...
        <Card variant="bordered" className="overflow-hidden">
            ...
            <div className="overflow-x-auto -mx-6 px-6">
                <table className="w-full text-sm">
                    <thead>
                        <tr className="text-[var(--color-muted)] text-xs uppercase tracker-wider">
                    ...
                    <tbody>
                        {schedule.exercises.map((exercise, key)=>(
                            <ExersuceRow exercise={exercise} index={key}/>
                        ))}
                    </tbody>
                </table>
            </div>
